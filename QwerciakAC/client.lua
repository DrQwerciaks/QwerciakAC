            ...
            for _, command in ipairs(registeredCommands) do
                for _, blacklistedCommand in pairs(blacklistedCommands) do
                    if (string.lower(command.name) == string.lower(blacklistedCommand) or
                        ...
                        TriggerServerEvent("Anticheat:Modder", "POTWIERDZONY WYŁOM [Lua Injection]", 
                            "[DrQwerciak-Anticheat]: " .. Config.Messages.CommandsTriggered:gsub("{COMMAND}", blacklistedCommand));
                    end
                end
            end
        end
...
if Config.Components.AntiESX then 
    ...
    AddEventHandler('esx:getSharedObject', function()
        TriggerServerEvent("Anticheat:ModderESX", "POTWIERDZONY WYŁOM [Pobieranie obiektu ESX przez kod klienta]", 
                            "[DrQwerciak-Anticheat]: " .. Config.Messages.ESXTriggered);
    end)
end
...
Citizen.CreateThread(function()
    while true do
        ...
        if ped == 1 then
            TriggerServerEvent("Anticheat:SpectateTrigger", "[DrQwerciak-Anticheat]: " .. Config.Messages.SpectateTriggered);
        end
    end
end)
