-- Server Config:
webhookURL = 'https://discordapp.com/api/webhooks/1118260251595116705/mFwGeRYnSfT2XoYWSEsnp2FNS-p5gGyRWl8XG6WnP4WrbKRqEetYTM53vYpTAemMvI56'
...
AddEventHandler('Anticheat:ScreenshotSubmit', function()
    ...
    if not IsPlayerAceAllowed(src, "Anticheat.Bypass") then 
        ...
        exports['discord-screenshot']:requestCustomClientScreenshotUploadToDiscord(src, webhookURL, screenshotOptions, {
            username = '[DrQwerciak-ANTICHEAT] https://github.com/JaredScar/DrQwerciak-Anticheat',
            avatar_url = 'https://forum.cfx.re/user_avatar/forum.cfx.re/officialDrQwerciak/120/258293_2.png',
            content = '',
            embeds = {
                {
                    color = 16711680,
                    author = {
                        name = '[DrQwerciak-ANTICHEAT]',
                        icon_url = 'https://forum.cfx.re/user_avatar/forum.cfx.re/officialDrQwerciak/120/258293_2.png'
                    },
...
RegisterCommand('ac-unban', function(source, args, rawCommand)
    ...
    if (src <= 0) then
        ...
        if #args == 0 then 
            ...
            print('^3[^6DrQwerciak-Anticheat^3] ^1Not enough arguments...');
            return; 
        end
        local banID = args[1];
        if tonumber(banID) ~= nil then
            ...
            if playerName then
                print('^3[^6DrQwerciak-Anticheat^3] ^0Player ^1' .. playerName 
                .. ' ^0has been unbanned from the server by ^2CONSOLE');
                TriggerClientEvent('chatMessage', -1, '^3[^6DrQwerciak-Anticheat^3] ^0Player ^1' .. playerName 
                .. ' ^0has been unbanned from the server by ^2CONSOLE'); 
            else 
                -- Not a valid ban ID
                print('^3[^6DrQwerciak-Anticheat^3] ^1That is not a valid ban ID. No one has been unbanned!'); 
            end
        end
        return;
    end 
    if IsPlayerAceAllowed(src, "DrQwerciak-Anticheat.ACban") then 
        if #args == 0 then 
            -- Not enough arguments
            TriggerClientEvent('chatMessage', src, '^3[^6DrQwerciak-Anticheat^3] ^1Not enough arguments...');
            return; 
        end
        local banID = args[1];
        if tonumber(banID) ~= nil then 
            ...
            if playerName then
                ...
                -- Not a valid ban ID
                TriggerClientEvent('chatMessage', src, '^3[^6DrQwerciak-Anticheat^3] ^1That is not a valid ban ID. No one has been unbanned!'); 
            end
        else 
            -- Not a valid number
            TriggerClientEvent('chatMessage', src, '^3[^6DrQwerciak-Anticheat^3] ^1That is not a valid number...'); 
        end
    end
end)
...
RegisterCommand("latest", function(source, args, rawCommand) 
    ...
    for i = 1, #latestUsers do 
        ...
        TriggerClientEvent('chatMessage', source, "^5[^1DrQwerciak-Anticheat^5] ^3Player ^3[^4".. tostring(user) .. "^3] ^4" .. 
...
Citizen.CreateThread(function()
    while true do 
        ...
        for _, id in pairs(GetPlayers()) do 
            if isBanned(id) then 
                -- Banned, kick em 
                DropPlayer(id, "[DrQwerciak-Anticheat] " .. bans[tostring(playerIP)]);
            end
        end
    end
end)
...
function OnPlayerConnecting(name, setKickReason, deferrals)
    deferrals.defer();
    print("[DrQwerciak-Anticheat] Checking their Ban Data");
    local src = source;
    local banned = false;
    ...
    if ban then 
        ...
        if string.find(reason, "[DrQwerciak-Anticheat]") then 
            printMessage = "" 
        else 
            printMessage = "[DrQwerciak-Anticheat] " 
        end 
...
RegisterCommand("acban", function(source, args, raw)
    ...
    if IsPlayerAceAllowed(src, "DrQwerciak-Anticheat.ACban") then 
        ...
        if #args < 2 then 
            ...
            TriggerClientEvent('chatMessage', source, "^5[^1DrQwerciak-Anticheat^5] ^1ERROR: You have supplied invalid amount of arguments... " ..
        ...
        if ExtractIdentifiers(args[1]) ~= nil then 
            ...
            DropPlayer(id, "[DrQwerciak-Anticheat]: Banned by player " .. GetPlayerName(src) .. " for reason: " .. reason);
        else 
            -- Not a valid player supplied 
            TriggerClientEvent('chatMessage', source, "^5[^1DrQwerciak-Anticheat^5] ^1ERROR: There is no valid player with that ID online... " ..
                "^2Proper Usage: /acban <id> <reason>");
        end
    end
end)
    ...
    if Config.Components.AntiNoclip and not IsPlayerAceAllowed(source, "Anticheat.Bypass") then
        ...
        if counter[ids.steam] ~= nil and counter[ids.steam] >= Config.NoClipTriggerCount then 
            ...
            if (Config.BanComponents.AntiNoClip) then 
                ...
                sendToDisc("[BANNED] POTWIERDZONY WYŁOM (NoClipping around): _[" .. tostring(id) .. "] " .. GetPlayerName(id) .. "_", 
                'Steam: **' .. steam .. '**\n' ..
                'GameLicense: **' .. gameLicense .. '**\n' ..
                'Discord Tag: **<@' .. discord:gsub('discord:', '') .. '>**\n' ..
                'Discord UID: **' .. discord:gsub('discord:', '') .. '**\n');
            else 
                sendToDisc("POTWIERDZONY WYŁOM [NoClipping around]: _[" .. tostring(id) .. "] " .. GetPlayerName(id) .. "_", 
                'Steam: **' .. steam .. '**\n' ..
                'GameLicense: **' .. gameLicense .. '**\n' ..
                'Discord Tag: **<@' .. discord:gsub('discord:', '') .. '>**\n' ..
                'Discord UID: **' .. discord:gsub('discord:', '') .. '**\n');
            end
            DropPlayer(id, "[DrQwerciak-Anticheat]: " .. Config.Messages.NoClipTriggered)
        end 
        Wait(6000);
        counter[ids.steam] = counter[ids.steam] - 1;
    end
...
AddEventHandler("Anticheat:SpectateTrigger", function(reason)
    if Config.Components.AntiSpectate and not IsPlayerAceAllowed(source, "Anticheat.Bypass") then 
        ...
        if Config.BanComponents.AntiSpectate then 
            BanPlayer(id, reason);
            sendToDisc("[BANNED] POTWIERDZONY WYŁOM (Tried spectating a player): _[" .. tostring(id) .. "] " .. GetPlayerName(id) .. "_", 
            'Steam: **' .. steam .. '**\n' ..
            'GameLicense: **' .. gameLicense .. '**\n' ..
            'Discord Tag: **<@' .. discord:gsub('discord:', '') .. '>**\n' ..
            'Discord UID: **' .. discord:gsub('discord:', '') .. '**\n');
        else 
            sendToDisc("POTWIERDZONY WYŁOM [Tried spectating a player]: _[" .. tostring(id) .. "] " .. GetPlayerName(id) .. "_", 
            'Steam: **' .. steam .. '**\n' ..
            'GameLicense: **' .. gameLicense .. '**\n' ..
            'Discord Tag: **<@' .. discord:gsub('discord:', '') .. '>**\n' ..
            'Discord UID: **' .. discord:gsub('discord:', '') .. '**\n');
...
AddEventHandler('chatMessage', function(source, name, msg)
    ...
    if (name ~= realName) then 
        if Config.BanComponents.AntiFakeMessage then 
            BanPlayer(id, "[DrQwerciak-Anticheat]: " .. Config.Messages.ChatMessageTriggered)
            sendToDisc("[BANNED] POTWIERDZONY WYŁOM (Fake Chat Message): _[" .. tostring(id) .. "] " .. GetPlayerName(id) .. "_", 
            'Steam: **' .. steam .. '**\n' ..
            'GameLicense: **' .. gameLicense .. '**\n' ..
            'Discord Tag: **<@' .. discord:gsub('discord:', '') .. '>**\n' ..
            'Discord UID: **' .. discord:gsub('discord:', '') .. '**\n'
            ...
            sendToDisc("POTWIERDZONY WYŁOM [Fake Chat Message]: _[" .. tostring(id) .. "] " .. GetPlayerName(id) .. "_", 
            'Steam: **' .. steam .. '**\n' ..
            'GameLicense: **' .. gameLicense .. '**\n' ..
            'Discord Tag: **<@' .. discord:gsub('discord:', '') .. '>**\n' ..
            'Discord UID: **' .. discord:gsub('discord:', '') .. '**\n'
        ...
        DropPlayer(id, "[DrQwerciak-Anticheat]: " .. Config.Messages.ChatMessageTriggered)
    end
end)
  ...
  RegisterServerEvent(BlacklistedEvents[i])
    AddEventHandler(BlacklistedEvents[i], function()
        ...
        if Config.BanComponents.AntiBlacklistedEvent then 
            BanPlayer(id, "[DrQwerciak-Anticheat]: " .. reason);
            sendToDisc("[BANNED] POTWIERDZONY WYŁOM (Tried executing `".. BlacklistedEvents[i] .."`): _[" .. tostring(id) .. "] " .. GetPlayerName(id) .. "_", 
            'Steam: **' .. steam .. '**\n' ..
            'GameLicense: **' .. gameLicense .. '**\n' ..
            'Discord Tag: **<@' .. discord:gsub('discord:', '') .. '>**\n' ..
            'Discord UID: **' .. discord:gsub('discord:', '') .. '**\n');
        else 
            sendToDisc("POTWIERDZONY WYŁOM [Tried executing `".. BlacklistedEvents[i] .."`]: _[" .. tostring(id) .. "] " .. GetPlayerName(id) .. "_", 
            'Steam: **' .. steam .. '**\n' ..
            'GameLicense: **' .. gameLicense .. '**\n' ..
            'Discord Tag: **<@' .. discord:gsub('discord:', '') .. '>**\n' ..
            'Discord UID: **' .. discord:gsub('discord:', '') .. '**\n');
        end
      DropPlayer(id, "[DrQwerciak-Anticheat]: " .. reason)
    end)
end
...
AddEventHandler("entityCreating",  function(entity)
    ...
    if (model) then 
        if (owner ~= nil and owner > 0) then
            ...
            DropPlayer(owner, "[DrQwerciak-Anticheat]: " .. reason);
        end
        CancelEvent()
        cancelled = true
    end
...
CreateThread(function()
    if Config.Components.ModMenuChecks then
        ...
        for i = 1, GetNumResources() do
            ...
            if resource_name ~= GetCurrentResourceName() then
                for k, v in pairs({'fxmanifest.lua', '__resource.lua'}) do
                    ...
                    if data and type(data) == 'string' and string.find(data, 'acloader.lua') ~= nil then
                       -- data = data:lower()
                        local removed = string.gsub(data, 'client_script "%@DrQwerciak%-anticheat%-master%/acloader.lua"', "")
        ...
        if added then
            print('[DrQwerciak-AC] Uninstall Mod-Menu-Checks | Modified 1 or more resources. It is required to restart your server so these changes can now take place.')
        end
    end
end)
    ...
    AddEventHandler("ANTICHEAT:CHECKRESOURCES", function(givenList)
        ...
        for _, resource in ipairs(givenList) do
            if not validResourceList[resource] then
                BanPlayer(source, "[DrQwerciak-Anticheat]: " .. Config.Messages.UnauthorizedResources:gsub("{RESOURCE}", resource));
            end
        end
    end)
end
